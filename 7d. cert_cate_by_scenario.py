...
def main():
    ...
    for s in scenarios:
        ...
        n_ins = n_insiders[s]
        print(f"{scenario_labels[s]} [n={n_ins} insiders, {len(d)}/{N_BOOTSTRAP} valid replicates]:")
        if abs(ci_high - ci_low) < 0.001:
            print(f"    No detectable effect at this sample size (point estimate {ate:+.4f}, "
                  f"but the bootstrap distribution is too narrow/sparse at n={n_ins} insiders to "
                  f"resolve a confidence interval with meaningful precision -- report as 'no "
                  f"detectable effect', not as a near-zero-width CI).")
        else:
